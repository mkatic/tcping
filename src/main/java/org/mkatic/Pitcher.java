...
class Pitcher {
  ...
  private class PitcherChannelHandler extends SimpleChannelInboundHandler<ByteBuf> {
    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, ByteBuf in) {
      ...
      try {
        ...
        System.out.println("Interrupted while queueing response");
        System.exit(-1);
      }
    }
  ...
  /**
   * Implementation of one way delay calculations is trivial on purpose. One way delay measurements are notoriously difficult to do, if not
   * even impossible to do without extra hardware, like external clocks. And even more so if we only have control of endpoints.
   *
   * One might try to sync catcher and pitcher via NTP, but this will probably not work well in all situations, especially if you're dealing
  ...
  private class PingSpammer implements Runnable {
    ...
    private long loopStart = System.currentTimeMillis();
      ...
      while (!Thread.currentThread().isInterrupted()) {
        try {
          if ((System.currentTimeMillis() - loopStart) <= 1000) {
            ...
            ChannelFuture cf = ctx.writeAndFlush(Unpooled.copiedBuffer(new TcpingPacket(seqNo, System.currentTimeMillis()).getBytes()));
            response = responseQ.poll(10, TimeUnit.SECONDS);
            cf.syncUninterruptibly();
            if (response == null) {
              System.out.println("No response for packet: " + seqNo);
            } else {
              response.setReceivedTimestamp(System.currentTimeMillis());
              receivedPacketList.add(response);
              sentCounter++;
            }
            seqNo++;
            ...
            System.out.printf("%s |Total received: %8d|msg/s %6d|max RTT: %4dms|avg RTT: %4.2fms|avg RTT AB: %4.2fms|avg RTT BA: %4.2fms\n",
                dtf.format(time), sentCounter, rttList.size(), rttMax, BigDecimal.valueOf(rttAvg).setScale(2, HALF_UP), rttAvgSym,
                rttAvgSym);
